...
This way you can distribute both the address of a router within Mycoria __and__ its public key with a single data point: An IPv6 address. As pretty much any software is able to resolve domains to and connect via IPv6, Mycoria is compatible with all existing Internet infrastructure, as long as you have Mycoria running on your device.

_This enables for Mycoria to do 100% end-to-end encryption of traffic within the network._
...
_Note: Switch labels can reveal some information if enough of the network is known. We are thinking about way to mask the switch labels to mitigate this._
